...
export default function LabDashboard() {
    ...
    const StatCard = ({ title, value, icon: Icon, color, subValue }: any) => (
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm relative overflow-hidden group">
            <div className={`absolute top-0 right-0 w-24 h-24 -mr-8 -mt-8 bg-${color}-50 rounded-full group-hover:scale-110 transition-transform duration-500 opacity-50`}></div>
            <div className="flex items-start justify-between relative z-10">
                <div>
                    <p className="text-sm font-medium text-gray-400 uppercase tracking-wider mb-1">{title}</p>
                    <h3 className="text-3xl font-black text-gray-900">{value}</h3>
                    {subValue && <p className="text-xs text-gray-500 mt-2 font-semibold flex items-center gap-1">
        ...
        <div className="p-8 max-w-[1600px] mx-auto min-h-screen bg-gray-50/30">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
                <div>
                    ...
                    <p className="text-gray-500 font-medium mt-1">Real-time laboratory diagnostics & performance tracking</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <h4 className="text-xl font-black text-gray-900">₹{stats?.paymentBreakdown.Cash.toLocaleString() || 0}</h4>
                    </div>
                </div>
                <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <h4 className="text-xl font-black text-gray-900">₹{stats?.paymentBreakdown.UPI.toLocaleString() || 0}</h4>
                    </div>
                </div>
                <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <h4 className="text-xl font-black text-gray-900">₹{stats?.paymentBreakdown.Card.toLocaleString() || 0}</h4>
                    </div>
                </div>
            </div>
