...
export default function PrescriptionPage() {
    ...
    return (
        <div className="bg-gray-50 min-h-screen pb-20 font-sans">
            ...
            {/* --- PRINT LAYOUT --- */}
            <div className="hidden print:block bg-white relative text-gray-900 overflow-hidden font-sans print-prescription-wrapper" 
                 style={{ 
                    width: '210mm', 
                    height: '296mm', // Slightly less than 297 to avoid overflow
                    margin: '0 auto', 
                    padding: '12mm 15mm 12mm 25mm', // Tightened padding
                ...
                <div className="flex justify-between items-start mb-6 border-b-2 border-blue-500 pb-4">
                    <div className="flex gap-4">
                        <div className="w-16 h-16">
                            <img src="/logo.png" alt="Logo" className="w-full h-full object-contain opacity-90" onError={(e) => (e.currentTarget.src = 'https://via.placeholder.com/150')} />
                        </div>
                        <div>
                            <h1 className="text-2xl font-black text-blue-600 uppercase tracking-tighter italic leading-none">PRESCRIPTION</h1>
                            <p className="text-[8px] font-bold text-gray-400 uppercase tracking-widest mt-1">Medical Record Node • Verified Registry</p>
                            <div className="mt-2">
                                <h2 className="font-black text-lg text-gray-900 uppercase leading-none">RIMS Government General Hospital Kadapa</h2>
                                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-tight mt-1">RIMS Road, Putlampalli, Kadapa, Andhra Pradesh</p>
                            </div>
                        </div>
                    </div>
                    <div className="text-right flex flex-col items-end gap-1">
                        <div className="bg-gray-900 text-white px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest mb-1">Official Document</div>
                        <p className="text-[10px] font-black text-gray-900 uppercase">Date: <span className="font-medium">{formData.date}</span></p>
                        <p className="text-[10px] font-black text-gray-900 uppercase tracking-tighter">Ref ID: <span className="font-medium text-blue-600">{formData.mrn}</span></p>
                    </div>
                </div>

                {/* Patient / Doctor Summary Card - Now more compact */}
                <div className="bg-blue-50/40 rounded-xl p-6 mb-6 grid grid-cols-2 gap-x-8 gap-y-3 text-xs border border-blue-100/30">
                    <div className="space-y-1">
                        <p className="text-[8px] font-black text-blue-600 uppercase tracking-widest">Patient Details</p>
                        <p className="text-base font-black text-gray-900 uppercase tracking-tight border-b border-gray-200 inline-block">{formData.patientName}</p>
                        <p className="font-bold text-gray-600 text-[10px]">{formData.age} Y • {formData.gender}</p>
                    </div>
                    <div className="space-y-1">
                        <p className="text-[8px] font-black text-blue-600 uppercase tracking-widest">Prescribing Physician</p>
                        <p className="text-base font-black text-gray-900 uppercase tracking-tight">Dr. {formData.doctorName}</p>
                        <p className="font-bold text-gray-600 text-[10px]">MBBS, MD Internal Medicine</p>
                    </div>

                    <div className="col-span-2 space-y-1 mt-1 border-t border-blue-100/50 pt-2">
                        <p className="text-[8px] font-black text-blue-600 uppercase tracking-widest">Clinical Diagnosis</p>
                        <p className="text-[11px] font-bold text-gray-800 italic leading-snug">"{formData.diagnosis || formData.symptoms || 'General Consultation & Follow-up'}"</p>
                    </div>
                </div>

                {/* Medicines List - Robust Table */}
                <div className="mb-6">
                    <h3 className="text-blue-600 text-[10px] font-black uppercase tracking-widest mb-4 flex items-center gap-2">
                        <span className="w-5 h-5 rounded bg-blue-600 text-white flex items-center justify-center text-[8px] italic font-black">Rx</span>
                        Medication Protocol
                    </h3>
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-gray-200 text-gray-400">
                                <th className="py-1 text-[8px] font-black uppercase">Medicine & Form</th>
                                <th className="py-1 text-[8px] font-black uppercase text-center">Dosage / Frequency</th>
                                <th className="py-1 text-[8px] font-black uppercase text-right">Duration</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-50">
                            {formData.medicines.map((med, idx) => (
                                <tr key={idx}>
                                    <td className="py-3">
                                        <p className="text-[11px] font-black text-gray-900 uppercase leading-none">{med.name}</p>
                                        {med.notes && <p className="text-[9px] text-gray-400 font-bold mt-1 leading-none italic">{med.notes}</p>}
                                    </td>
                                    <td className="py-3 text-center">
                                        <p className="text-[10px] font-bold text-gray-700 leading-none">{med.dosage}</p>
                                        <p className="text-[8px] text-blue-600 font-black mt-1 uppercase">{med.freq}</p>
                                    </td>
                                    <td className="py-3 text-right text-[10px] font-black text-gray-900 uppercase">
                                        {med.duration}
                                    </td>
                                </tr>
                            ))}
                ...
                <div className="grid grid-cols-2 gap-8 mb-6">
                    <div>
                        <h3 className="text-blue-600 text-[9px] font-black uppercase tracking-widest mb-3 border-b border-gray-100 pb-1">Dietary Regimen</h3>
                        <ul className="space-y-1.5">
                            {formData.dietAdvice.map((item, idx) => (
                                <li key={idx} className="text-[10px] font-bold text-gray-700 flex items-start gap-1.5">
                                    <div className="w-1 h-1 rounded-full bg-blue-400 mt-1" /> {item}
                                </li>
                            ))}
                        </ul>
                    </div>
                    <div>
                        <h3 className="text-blue-600 text-[9px] font-black uppercase tracking-widest mb-3 border-b border-gray-100 pb-1">Laboratory Investigations</h3>
                        <ul className="space-y-1.5">
                ...
                <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 mb-6">
                    <div className="grid grid-cols-[100px_1fr] gap-2 mb-2">
                        <span className="text-[9px] font-black text-blue-600 uppercase italic">Follow Up:</span>
                        <span className="text-[10px] font-black text-gray-800 uppercase">{formData.followUp || 'As instructed by physician'}</span>
                    </div>
                    {formData.avoid.length > 0 && (
                        <div className="grid grid-cols-[100px_1fr] gap-2">
                            <span className="text-[9px] font-black text-rose-600 uppercase italic">Avoid:</span>
                            <span className="text-[10px] font-bold text-gray-700 uppercase leading-tight">{formData.avoid.join(', ')}</span>
                        </div>
                    )}
                </div>
                ...
                <div className="absolute bottom-24 right-12 w-48 text-center">
                    <div className="h-16 flex items-end justify-center mb-1">
                        {formData.doctorSignature ? (
                             ...
                             <span className="font-handwriting text-2xl text-blue-900 font-bold">Dr. {formData.doctorName}</span>
                        )}
                    </div>
                    <div className="border-t border-gray-900 pt-1">
                        <p className="text-[9px] font-black text-gray-900 uppercase tracking-widest">Medical Officer</p>
                        <p className="text-[7px] font-bold text-gray-400 mt-0.5 uppercase tracking-tighter">Verified Digital Authentication</p>
                    </div>
                </div>

                {/* Footer Trace */}
                <div className="absolute bottom-8 left-[25mm] right-15mm border-t border-gray-100 pt-3 flex justify-between items-center text-[7px] text-gray-300 font-black uppercase tracking-widest">
                    <p>CURECHAIN DIGITAL HEALTH SYSTEM • GEN: {new Date().toLocaleDateString('en-GB')} {new Date().toLocaleTimeString('en-GB')}</p>
                    <p>ELECTRONIC PRESCRIPTION • PAGE 1 OF 1</p>
                </div>

                {/* Diagonal Watermark - Softer */}
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 -rotate-45 pointer-events-none opacity-[0.03] select-none text-8xl font-black text-blue-900 whitespace-nowrap tracking-tighter italic uppercase">
                    RIMS Govt Hospital
                </div>
            </div>
        ...
        @media print {
            ...
            .print\\:hidden, nav, header, aside, .sidebar, .navbar, #navbar, #sidebar, .no-print, button {
