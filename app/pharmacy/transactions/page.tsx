                            ...
                            {loading ? (
                                ...
                                bills.map((bill) => (
                                    <tr key={bill._id} className="hover:bg-gray-50/50 transition-colors group">
                                        ...
                                        <td className="px-8 py-5">
                                            <p className="text-[13px] font-black text-gray-700 dark:text-white tracking-tight uppercase">{bill.patientName || 'Anonymous'}</p>
                                            <p className="text-[11px] font-bold text-gray-400">{bill.customerPhone || '-'}</p>
                                        </td>
                                        <td className="px-8 py-5 text-center">
                                            <div className="flex flex-col items-center gap-1">
                                                ...
                                                {bill.paymentSummary.paymentMode === 'Mixed' && bill.paymentSummary.paymentDetails && (
                                                    <span className="text-[9px] font-bold text-gray-400 lowercase">
                                                        ({Object.entries(bill.paymentSummary.paymentDetails)
                                                            .filter(([_, val]) => val > 0)
                                                            .map(([key]) => key)
                                                            .join(', ')})
                                                    </span>
